...
const ConfirmOrder = () => {
  ...
  const submitHandler = (e) => {
    e.preventDefault();
    if (couponNames.includes(couponCode)) {
      toast.error("Cannot add the same coupon code again")
      return
    }
    if (data?.coupon?.discountType === "fixed") {
      if (totalPrice < data?.coupon?.discountValue) {
        toast.error(`You need to purchase more than ₹ ${data?.coupon?.discountValue + 200} to apply this coupon`);
        return; 
       }
    }
    if (data?.coupon?.discountType === "percentage") {
      const discountedTotal = data?.coupon?.discountValue * totalPrice / 100;
      if (totalPrice <= discountedTotal) {
        toast.error(`Invalid coupon`);
        return; 
       }
    }
    checkCoupon({ code: couponCode })
    // setCouponNames((prevCouponNames) => [...prevCouponNames, couponCode]);
    // setCouponCode(null)
    ...
    if (isSuccess && !discountApplied) {
     ...
     setCouponCode('')
      if (data?.coupon?.discountType === "percentage") {
        ...
        const discountedAmount = totalPrice * discountFactor;
        const roundedDiscountedAmount = parseFloat(discountedAmount.toFixed(2));
        setCouponValue((prevCouponValue) => prevCouponValue + roundedDiscountedAmount);
        calculateOrderCostWithOffer(cartItems, couponValue + roundedDiscountedAmount);
  ...
  }, [data, isSuccess, totalPrice, discountApplied, error, cartItems, couponValue, couponCode]);
            ...
            <div className="w-full max-w-sm md:max-w-3xl xl:max-w-sm flex items-start flex-col gap-8 max-xl:mx-auto ">
              <div className="p-6 border border-gray-200 rounded-3xl bg-gray-950 w-full group transition-all duration-500 hover:border-gray-400 text-gray-200">
                  ...
                  <div className="flex items-center justify-between gap-4 mb-5">
                    ...
                    <p className="font-medium text-lg leading-8">
                      ...
                      {(
                        ...
                        Math.trunc(Number(totalPrice) || 0)
                      ).toFixed(2)}
                    </p>
                  </div>
                  <div className="flex items-center justify-between gap-4 mb-5">
                ...
                <div className=" w-full mt-4">
                  <PaymentMethods onMethodSelection={handleButtonClick} couponValue={couponValue} couponCodeNames={couponNames} />
                </div>
              </div>
            </div>
